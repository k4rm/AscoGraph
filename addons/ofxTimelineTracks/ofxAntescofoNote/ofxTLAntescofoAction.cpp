...
int ofxTLAntescofoAction::update_sub(ActionGroup *ag)
{
	...
	for (g = ag->sons.begin(); g != ag->sons.end(); g++) {
		...
		if ((gr = dynamic_cast<ActionGroup*>(*g))) {
			if (!gr->header->top_level_group) {
				gr->header->rect.x = ag->header->rect.x;
				if (gr->header->delay)
			...
			: header(header_), gfwd(g), event(e)
{
	if (g) {
		...
		for (i = g->actions().begin(); i != g->actions().end(); i++) {
			...
			if (gg) {
				ActionGroupHeader* nh = new ActionGroupHeader(header->beatnum + get_delay(gg), gg, event);
				ActionGroup *ag = new ActionGroup(gg, event, nh);
				sons.push_back(ag);
			}
...
float ActionGroup::get_delay(Action* tmpa) 
{
	float d = 0.;
	if (tmpa && tmpa->delay() && tmpa->delay()->value()) {
		d = (float)tmpa->delay()->eval();
	}
	return d;
}
...
ActionMessage::ActionMessage(Message* m, Event *e, ActionGroupHeader* header_) 
{
	...
	oss << *m;
	action = oss.str();
	cout << "Action: adding message : " << action <<  endl;
	//header->hidden = false;
...
void ActionGroupHeader::draw(ofxTLAntescofoAction *tlAction) 
{
	...
	/*
	rect.x = tlAction.bounds.x + normalizedXtoScreenX( timeline->beatToNormalizedX(act->beatnum), zoomBounds);
	rect.y = bounds.y + 3;
	rect.width = normalizedXtoScreenX( timeline->beatToNormalizedX(act->duration), zoomBounds);
	*/
