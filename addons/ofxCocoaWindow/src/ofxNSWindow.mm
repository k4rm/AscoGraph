...
name(name), frameRate(frameRate), isFullscreen(false) {
	...
	cout << "ofxNSWindow::ofxNSWindow: name:" << name << " options:" << options << " framerate:" << frameRate<< endl;
	//glview = [[OpenGLView alloc] initWithFrame:nsframe :app :frameRate];
	glview = [[GLView alloc] initWithFrame:nsframe :app :frameRate];
